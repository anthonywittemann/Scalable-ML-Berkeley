# Databricks notebook source exported at Fri, 24 Jul 2015 06:42:54 UTC
# MAGIC %md
...
aSparse = SparseVector(len(aDense), {1: 3.0, 3: 4.0})
...
sampleOneOHEFeatManual = SparseVector(len(sampleOHEDictManual), {2: 1.0, 3:1.0}) #[0,0,1,1,0,0,0]
sampleTwoOHEFeatManual = SparseVector(len(sampleOHEDictManual), {1: 1.0, 4:1.0, 5: 1.0}) #[0,1,0,0,1,1,0]
sampleThreeOHEFeatManual = SparseVector(len(sampleOHEDictManual), {0: 1.0, 3:1.0, 6: 1.0}) #[1,0,0,1,0,0,1]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    sparseDict = {}
    #for key,val in OHEDict.iteritems():
    #  for feat in rawFeats:
    #    if key == feat:
    #      sparseDict.update({val: 1.0})
    #(Dictionary[x],1.0) for x in features if x in Dictionary
    [sparseDict.update({OHEDict[x]:1.0}) for x in rawFeats if x in OHEDict]
    #[({val:1.0}) for key, val in OHEDict.iteritems() for feat in rawFeats if key == feat]
...
numSampleOHEFeats = len(sampleOHEDictManual) # working
...
def parsePoint(point):
    ...
    features = features[1:] #take out the label
    featureID = 0
    retList = []
    for featureVal in features:
      retList.append( (featureID, featureVal) )
      featureID += 1
    return retList
...
ctrOHEDict = createOneHotDict(parsedTrainFeat) 
			# what the above fn call is doing
			#(parsedTrainFeat
            #  		.flatMap(lambda x: x[:])
            #        .distinct()
            #        .zipWithIndex()
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    label = point[0]
    features = point[1:]
    parsedFeatures = parsePoint(features)
    OHEfeats = oneHotEncoding(parsedFeatures, OHEDict, numOHEFeats)
    return LabeledPoint(label, OHEfeats)

#testOHE = (parseOHEPoint(rawTrainData.first(),ctrOHEDict, numCtrOHEFeats))
#print "testOHE:"
#print testOHE
...
# MAGIC #### We are now ready to train our first CTR classifier.  A natural classifier to use in this setting is logistic regression, since it models the probability of a click-through event rather than returning a binary response, and when working with rare events, probabilistic predictions are useful.  First use [LogisticRegressionWithSGD](https://spark.apache.org/docs/latest/api/python/pyspark.mllib.html#pyspark.mllib.classification.LogisticRegressionWithSGD) to train a model using `OHETrainData` with the given hyperparameter configuration.  `LogisticRegressionWithSGD` returns a [LogisticRegressionModel](https://spark.apache.org/docs/latest/api/python/pyspark.mllib.html#pyspark.mllib.regression.LogisticRegressionModel).  Next, use the `LogisticRegressionModel.weights` and `LogisticRegressionModel.intercept` attributes to print out the model's parameters.  Note that these are the names of the object's attributes and should be called using a syntax like `model.weights` for a given `model`.
...
def computeLogLoss(p, y):
    ...
    if y == 1:
      ...
      print 'Houston, we have a non binary label!!!'
...
def getP(x, w, intercept):
    ...
    return 1.0 / (1.0 + exp(-rawPrediction)) # apply sigmoid function
...
def evaluateResults(model, data):
    ...
    """
    logLossData = data.map(lambda x: computeLogLoss(getP(x.features, model.weights, model.intercept), x.label)).mean()
    return logLossData
...
print len(OHEValidationData.first().features)
print len(OHETrainData.first().features)

print "Should be same:"
minLen = OHETrainData.map(lambda lp: len(lp.features)).min()
print(minLen)
minLen = OHEValidationData.map(lambda lp: len(lp.features)).min()
print(minLen)

print "Test Sparse Vectors:"
maxIDX = OHETrainData.map(lambda lp: max(lp.features.indices)).max()
print("Should be 233285: {}".format(maxIDX))
maxIDX = OHEValidationData.map(lambda lp: max(lp.features.indices)).max()
print("Should be 233284: {}".format(maxIDX))
...
def computeSparsity(data, d, n):
    ...
    """
    meanObservations = data.map(lambda lp: len(lp.features.indices)).mean()
    return meanObservations / d
...
# MAGIC %md
# MAGIC #### ** (5d) Logistic model with hashed features **
# MAGIC #### Now let's train a logistic regression model using the hashed features. Run a grid search to find suitable hyperparameters for the hashed features, evaluating via log loss on the validation data. Note: This may take a few minutes to run. Use `1` and `10` for `stepSizes` and `1e-6` and `1e-3` for `regParams`.
...
# MAGIC %md
# MAGIC #### ** (5e) Evaluate on the test set **
# MAGIC #### Finally, evaluate the best model from Part (5d) on the test set.  Compare the resulting log loss with the baseline log loss on the test set, which can be computed in the same way that the validation log loss was computed in Part (4f).
